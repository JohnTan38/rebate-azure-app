...
if dataUpload is None:
        ...
        for file in dataUpload:
            if 'Week_' in file.name:
                ...
                df_transport = df_transport[['ContainerNumber', 'CarrierName', 'EventTime', 'COD_ATD', 'PSALOLO']]
                ...
                df_week_appended['Size'] = df_week_appended['Size'].str.extract('(\d+)')#.astype(int)
                df_week_appended.dropna(subset=["Size"]) 
                ...
                def add_overall_efficiency(df, overall_efficiency):
                    ...
                    new_row = pd.DataFrame({'Week': [f"Week {int(df['Week '].str.split('_').str[-1].iloc[-1])+0}"], 
